...
sub vcl_backend_response {
    ...
    # Log backend fetch result
    if (beresp.status >= 500) {
        # Log errors for debugging
        std.log("Backend fetch failed with status: " + beresp.status);
    }
...
# backend default {
#   .host = "magento";  # Replace with actual container name
#   .port = "80";       # Adjust port if different for Magento
#   .connect_timeout = 60s;
...
# sub vcl_recv {
#   # Cache common content types for efficiency
#   if (req.http.Accept ~ "(image|javascript|text/css)") {
#     set req.http.X-Cache-TTL = "3600";
#   } else {
#     # Avoid caching dynamic content or user-specific pages
